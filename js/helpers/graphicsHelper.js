...
export const graphicsHelper = {
    ...
    /*Метод "parseColor()" на основе строки, указывающей цветом в формате HEX или в формате RGBA, формирует массив 
    ...
    parseColor: function (color) {
        ...
        if (color.startsWith('#')) {
            ...
            то есть серый цвет.*/
            if (!/^rgba\(\d+,\s*\d+,\s*\d+,\s*[\d.]+\)$/.test(color)) { return [119, 119, 119, 0.5] };
            ...
            if (parts.length !== 4 || parts.some(part => part === "")) { return [119, 119, 119, 0.5] };
    ...
    /*Метод "generateGradientColors()" генерирует массив элементов, который обозначает последовательность оттенков от 
    светлого к темному или от темного к светлому, на основе базового цвета.
    ...
    Метод "generateGradientColors()" возвращает массив элементов, который обозначает последовательность оттенков от 
    светлого к темному или от темного к светлому.*/
    generateGradientColors: function (baseColor, gradientSteps, darkeningStep, isGradientReversed) {
        ...
        for (let i = 0; i < gradientSteps; i++) {
            ...
            5 / (7 - 1) = 5 / 6
            6 / (7 - 1) = 6 / 6 = 1*/
            const factor = i / (gradientSteps - 1); // От 0 (темный) до 1 (светлый)
    ...
    drawPolygonFromVerticesWithStepGradient: function (
        ...
        /*Создаем массив чисел от 1.0 до 1 - lastLayerFactor, равномерно распределенных для заданного количества шагов. 
        ...
        /*Отрисовываем слои внутри камня указанное в параметре "steps" раз. Каждый проход цикла создает один слой камня, 
        который немного меньше предыдущего.*/
        for (let i = 0; i < gradientSteps; i++) {
