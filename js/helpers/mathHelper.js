...
export const mathHelper = {
    ...
    /*Метод "getMinNumberFromArray()" находит наименьшее число в массиве чисел.
    
    Метод "getMinNumberFromArray()" принимает следующие параметры:
    1. "array" - это параметр в виде массива, содержащий числа.   

    Метод "getMinNumberFromArray()" возвращает наименьшее число из массива чисел.*/
    getMinNumberFromArray: function (array) {
    ...
    /*Метод "getMaxNumberFromArray()" находит наибольшее число в массиве чисел.
    
    Метод "getMaxNumberFromArray()" принимает следующие параметры:
    1. "array" - это параметр в виде массива, содержащий числа.   

    Метод "getMaxNumberFromArray()" возвращает наибольшее число из массива чисел.*/
    getMaxNumberFromArray: function (array) {
        if (array.length < 1) { return null };
        if (array.length === 1) { return array[0] };
        let max = array[0];
    ...
    /*Метод "getArrayWithoutDuplicates()" формирует массив из элементов переденного массива, исключая дубликаты.
    
    Метод "getArrayWithoutDuplicates()" принимает следующие параметры:
    1. "array" - это параметр в виде массива, содержащего какие-либо элементы.   

    Метод "getArrayWithoutDuplicates()" возвращает массив из элементов переденного массива без дубликатов.*/
    getArrayWithoutDuplicates: function (array) {
        const uniqueArray = [];
        for (let i = 0; i < array.length; i++) { if (!uniqueArray.includes(array[i])) { uniqueArray.push(array[i]) } };
        return uniqueArray;
    },

    /*Метод "getRandomElementFromArray()" выбирает случайный элемент в массиве.
    
    Метод "getRandomElementFromArray()" принимает следующие параметры:
    1. "array" - это параметр в виде массива, содержащего какие-либо элементы.   

    Метод "getRandomElementFromArray()" возвращает случайный элемент из массива.*/
    getRandomElementFromArray: function (array) {
        const randomIndex = Math.floor(Math.random() * array.length);
        return array[randomIndex];
    },

    /*Метод "quickSortNumberArray()" сортирует массив чисел в порядке возрастания методом быстрой сортировки.
    
    Метод "quickSortNumberArray()" принимает следующие параметры:
    1. "array" - это параметр в виде массива, содержащий числа.   

    Метод "quickSortNumberArray()" возвращает отсортированный в порядке возрастания массив чисел.*/
    quickSortNumberArray: function (array) {
        ...
        return this.quickSortNumberArray(less).concat(pivot, this.quickSortNumberArray(greater));
    },
    ...
    /*Метод "doTwoNotRotatedRectanglesIntersect()" определяет пересекаются ли два прямоугольника.
    
    Метод "doTwoNotRotatedRectanglesIntersect()" принимает следующие параметры:
    1. "minX1" - это числовой параметр, содержащий минимальную X-координату первого прямоугольника.
    2. "maxX1" - это числовой параметр, содержащий максимальную X-координату первого прямоугольника.
    3. "minY1" - это числовой параметр, содержащий минимальную Y-координату первого прямоугольника.
    4. "maxY1" - это числовой параметр, содержащий максимальную Y-координату первого прямоугольника.

    5. "minX2" - это числовой параметр, содержащий минимальную X-координату второго прямоугольника.    
    6. "maxX2" - это числовой параметр, содержащий максимальную X-координату второго прямоугольника.    
    7. "minY2" - это числовой параметр, содержащий минимальную Y-координату второго прямоугольника.    
    8. "maxY2" - это числовой параметр, содержащий максимальную Y-координату второго прямоугольника.
    
    Метод "doTwoNotRotatedRectanglesIntersect()" возвращает:
    1. true, как знак того, что два прямоугольника пересекаются.
    2. false, как знак того, что два прямоугольника не пересекаются.*/
    doTwoNotRotatedRectanglesIntersect: function (
    ...
    2. Перебираем все соседние пары вершин многоугольника.
    ...
    7. Перебираем точки на отрезке луча и точки на сторонах многоугольника, сохраняя все совпадающие точки в массив.
    ...
    isPointInsidePolygon: function (point, vertices) {
        ...
        const rayPolygonBoundingBoxIntersectionPoint = { x: maxX, y: point.y };
        ...
        const rayPoints = this.findIntPointsOnLineSegment(
            point.x, point.y,
            rayPolygonBoundingBoxIntersectionPoint.x, rayPolygonBoundingBoxIntersectionPoint.y
        );
        ...
        /*Внешним циклом for перебираем точки на отрезке луча.*/
        for (let i = 0; i < rayPoints.length; i++) {
            ...
            for (let j = 0; j < pointsOnPolygonEdges.length; j++) {
                /*Если взятая точка на отрезке луча совпадает со взятой точкой на стороне многоугольника, то добавляем
                ...
                if (rayPoints[i].x === pointsOnPolygonEdges[j].x && rayPoints[i].y === pointsOnPolygonEdges[j].y) {
                    ...
                    // ctx.fillStyle = 'rgba(229, 255, 0, 1)';
                    ...
                    тестирования.*/
                    // ctx.fillStyle = 'rgba(27, 112, 11, 1)';
    ...
    3. Если векторное произведение двух векторов AB и AP равно нулю, то это означает, что:
        ...
        2) Векторы AB и AP коллинеарные, то есть лежат на одной прямой.
    ...
    /*Метод "correctDiagonalMovementSpeed()" корректирует скорость объекта при диагональном движении.

    Когда объект движется по диагонали, его скорость должна быть распределена между X-направлением и Y-направлением. 
    Если просто установить одинаковую скорость по обеим осям, то фактическая скорость объекта будет больше, чем 
    задумано. Например, если скорость по оси X равна 5 и скорость по оси Y равна 5, то скорость при диагональном 
    движении по теореме Пифагора будет равна sqrt(5 * 5 + 5 * 5) ≈ 7.07.

    Чтобы решить эту проблему, нужно сделать следующее:
    1. Найти текущую (некорректную) величину скорости при диагональном движении путем нахождения длины исходного вектора 
    скорости при диагональном движении.
    2. Найти компоненты нормализованного вектора скорости путем деления каждой длины компоненты некорректного вектора на 
    длину некорректного вектора.
    3. Компоненты нормализованного вектора скорости умножить на требуемую скорость, чтобы получить компоненты 
    корректного вектора скорости.
    4. Полученные компоненты будут сообщать на сколько нужно пошагово сдвинуть объект по осям, чтобы объект прошел 
    корректное расстояние.

    Чтобы найти текущую (некорректную) величину скорости при диагональном движении нужно найти длину исходного вектора 
    скорости при диагональном движении. Если построить прямоугольный треугольник, где катеты будут равны текущим 
    скоростям по оси X и по оси Y, то длина гипотенузы, найденная по теореме Пифагора в этом треугольнике, будет равна 
    длине исходного вектора скорости при диагональном движении. Здесь нужно понимать, что при помощи этой гипотенузы 
    можно найти только длину исходного вектора скорости при диагональном движении, но не направление этого вектора.

    Нормализация вектора - это процесс приведения вектора к единичной длине, сохраняя его направление. Нормализованный 
    вектор можно умножить на какое-то число, чтобы указать "на сколько сильно надо двигаться" в каком-то направлении. 
    Чтобы произвести нормализацию исходного вектора скорости при диагональном движении, нужно длины его компонент 
    поделить на длину исходного вектора скорости при диагональном движении. Компоненты вектора это проекции этого 
    вектора на оси координат, то есть в нашем случае это вектора текущих скоростей по оси X и по оси Y.
    
    Чтобы получить компоненты правильного вектора скорости при диагональном движении, нужно компоненты нормализованного 
    исходного вектора скорости при диагональном движении умножить на требуемую скорость.

    Метод "correctDiagonalMovementSpeed()" принимает следующие параметры:
    1. "currentSpeedX" - это числовой параметр, указывающий текущую скорость передвижения объекта по оси X.
    2. "currentSpeedY" - это числовой параметр, указывающий текущую скорость передвижения объекта по оси Y.

    Метод "correctDiagonalMovementSpeed()" возвращает объект, содержащий скорости объекта по осям X и Y для корректной 
    скорости при диагональном движении.*/
    correctDiagonalMovementSpeed: function (currentSpeedX, currentSpeedY) {
        /*Вычисляем длину некорректного вектора скорости при диагональном движении.*/
        const length = Math.sqrt(currentSpeedX * currentSpeedX + currentSpeedY * currentSpeedY);
        /*Находим компоненты нормализованного вектора скорости при диагональном движении.*/
