...
export const mathHelper = {
    ...
    isPointInsidePolygon: function (point, vertices) {
        ...
        const isPointInsidePolygonBoundingBox = this.isPointInsideNotRotatedRectangle(
            minX, maxX,
            minY, maxY,
            point.x, point.y
        );
        ...
        поэтому возвращаем false, как знак того, что точка находится вне многоугольника.*/
        // if (
    ...
    getPointPositionRelativeToLineSegment: function (
        ...
        /*Если векторное произведение, учитывая погрешность, меньше нуля, то это означает, что точка находится справа от 
        прямой, содержащей отрезок. В этом случае вовзращаем -1, как знак того, что точка находится справа от прямой, 
        содержащей отрезок.*/
        return -1;
    },
    ...
    Геометрический смысл скалярного произведения двух векторов:
    1. Если скалярное произведение двух векторов положительное, то это означает, что векторы направлены в одну сторону.
    2. Если скалярное произведение двух векторов отрицательное, то это означает, что векторы направлены в 
    ...
    2. 1, как знак того, что векторы направлены в одну сторону.
    3. -1, как знак того, что векторы направлены в противоположные стороны.*/
    getVectorsRelativeDirection: function (
        ...
        перпендикулярны. В этом случае вовзращаем 0, как знак того, что векторы перпендикулярны.*/
        ...
        направлены в одну сторону. В этом случае вовзращаем 1, как знак того, что векторы направлены в одну сторону.*/
        ...
        направлены в противоположные стороны. В этом случае вовзращаем 1, как знак того, что векторы направлены в 
        противоположные стороны.*/
        return -1;
    },

    /*Метод "getSquaredVectorLength()" рассчитывает квадрат длины вектора, то скалярное произведение двух таких 
    векторов.
    ...
    1. Проверяем не лежит ли точка за пределами прямой, содержащей отрезок, при помощи метода 
    "helper.getPointPositionRelativeToLineSegment()".
    2. При помощи метода "helper.getDotProduct()" рассчитываем скалярное произведение двух направленных в одну сторону 
    векторов, где первый вектор строится на основе точек отрезка, а второй вектор строится из начальной точки первого 
    вектора в точку, которую проверяем находится ли она на отрезке.
    ...
    isPointOnLineSegment: function (
        ...
        "helper.getPointPositionRelativeToLineSegment()".*/
        const pointPositionRelativeToLineSegment = this.getPointPositionRelativeToLineSegment(
            startPointX, startPointY, endPointX, endPointY,
            pointX, pointY
        );

        /*Если точка не лежит на прямой, содержащей отрезок, то это означает, что она не лежит и на самом отрезке, 
        поэтому дальше нет смысла проводить проверку. Возвращаем false, как знак того, что точка не лежит на отрезке.*/
        if (pointPositionRelativeToLineSegment !== 0) { return false };

        /*При помощи метода "helper.getDotProduct()" рассчитываем скалярное произведение двух направленных в одну 
        сторону векторов, где первый вектор строится на основе точек отрезка, а второй вектор строится из начальной 
        точки первого вектора в точку, которую проверяем находится ли она на отрезке.*/
        const dotProduct = this.getDotProduct(
            startPointX, startPointY, endPointX, endPointY,
            startPointX, startPointY, pointX, pointY
        );
        ...
        векторы направлены в разные стороны. А это в свою очередь означает, что наша точка находится до начальной точки 
        первого вектора, то есть за пределами отрезка. Поэтому дальше нет смысла проводить проверку. Возвращаем false, 
        как знак того, что точка не лежит на отрезке.*/
        if (dotProduct < -1 * epsilon) { return false };

        /*Рассчитываем квадрат длины первого вектора при помощи метода "helper.getSquaredVectorLength()".*/
        const squaredLength = this.getSquaredVectorLength(startPointX, startPointY, endPointX, endPointY);

        /*Если два вектора выходят из одной точки и их скалярное произведение больше, чем квадрат длины левого вектора, 
        то это означает, что конечная точка правого вектора дальше конечной точки левого вектора.
    ...
    1. Определяем положение конечных точек отрезка относительно противоположный отрезков при помощи метода 
    "helper.getPointPositionRelativeToLineSegment()".
    2. Проверяем общий случай пересечения двух отрезков, когда конечные точки каждого отрезка лежат по разные стороны 
    относительно противоположного отрезка.
    ...
    doTwoLineSegmentsIntersect: function (point01, point02, point03, point04) {
        ...
        при помощи метода "helper.getPointPositionRelativeToLineSegment()". Результат проверки сохраняем в переменную 
        "point01Orientation".*/
        ...
        "helper.isPointOnLineSegment()" проверяем не лежит ли точка "point01" на самом отрезке "point03-point04". Если 
        это так, то это означает, что отрезок "point01-point02" касается отрезка "point03-point04" точкой "point01". 
        Тогда возвращаем true, как знак того, что отрезки пересекаются.*/
        ...
        "helper.isPointOnLineSegment()" проверяем не лежит ли точка "point02" на самом отрезке "point03-point04". Если 
        это так, то это означает, что отрезок "point01-point02" касается отрезка "point03-point04" точкой "point02". 
        Тогда возвращаем true, как знак того, что отрезки пересекаются.*/
        ...
        "helper.isPointOnLineSegment()" проверяем не лежит ли точка "point03" на самом отрезке "point01-point02". Если 
        это так, то это означает, что отрезок "point03-point04" касается отрезка "point01-point02" точкой "point03". 
        Тогда возвращаем true, как знак того, что отрезки пересекаются.*/
        ...
        "helper.isPointOnLineSegment()" проверяем не лежит ли точка "point04" на самом отрезке "point01-point02". Если 
        это так, то это означает, что отрезок "point03-point04" касается отрезка "point01-point02" точкой "point04". 
        Тогда возвращаем true, как знак того, что отрезки пересекаются.*/
    ...
    1. "vertices01" - это параметр в виде массива объектов, содержащих координаты вершин первого многоугольника.
    2. "vertices02" - это параметр в виде массива объектов, содержащих координаты вершин второго многоугольника.
    
    Логика работы метода "doTwoPolygonsIntersect()" следующая:
    1. Перебираем вершины многоугольников и ищем совпадения каких-нибудь вершин.
    2. Перебираем пары соседних вершин каждого многоугольника, чтобы на их основе проверять не пересекаются какие-то
    стороны этих многоугольников.
    3. Если пересечений сторон выявлено не было, то проверяем не находится ли полностью один многоугольник внутри 
    другого многоугольника.
    4. Если ни одна проверка не прошла, то возвращаем false, как знак того, что многоугольники не пересекаются.
    ...
    doTwoPolygonsIntersect: function (vertices01, vertices02) {
        ...
        первом многоугольнике. Каждая пара вершин будет конечными точками какой-то стороны многоугольника.*/
        for (let i = 0; i < vertices01.length; i++) {
            ...
            конечными точками какой-то стороны многоугольника.*/
            for (let j = 0; j < vertices02.length; j++) {
                ...
                "helper.doLineSegmentsIntersect()". Если это так, то возвращаем true, как знак того, что многоугольники
                пересекаются, поскольку если у двух многоугольников пересекаются хотя две стороны, то и сами 
                многоугольники пересекаются.*/
    ...
    doTwoPolygonsCoincide: function (vertices01, vertices02) {
        ...
        переберираем все вершины многоугольников и ищем совпадения между ними. Если количество совпадений вершин 
