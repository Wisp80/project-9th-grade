...
export const mathHelper = {
    ...
    /*Метод "isPointInsideNotRotatedRectangle()" определяет находится ли точка внутри неповернутого прямоугольника.
    ...
    1. true, как знак того, что точка находится внутри неповернутого прямоугольника.
    2. false, как знак того, что точка находится снаружи неповернутого прямоугольника.*/
    ...
    preparePolygonVerticesData: function (
        ...
        вершины многоугольника. Полученный результат сохраняем в переменную "veticesPerQuarterMoving". Это значение 
        нужно, чтобы переходить при рассчете координат вершин многоугольника из одной четверти области, в рамках которой 
        ...
        вершины многоугольника, мы проводим рассчет координат вершин многоугольника.*/
        ...
        for (let i = 1; i <= numberOfVertices; i++) {
                    ...
                    case 5:
                        {
                            ...
                            предыдущей рассчитанной вершины многоугольника. Эта проверка нужна, чтобы не было склеек 
            ...
            области из переменной "veticesPerQuarter". Такая логика позволяет рассчитывать примерно одинаковое 
    ...
    2. Находим точки с целочисленными координатами на каждой стороне многоугольника.
    ...
    1. Проверяем не совпадает ли точка с одной из вершин многоугольника.
    2. Проверяем не находится ли точка снаружу "ограничивающей коробки" многоугольника.
    3. Находим точку пересечения луча и правой (так как пускаем луч вправо) стороны холста.
    4. Находим точки с целочисленными координатами на отрезке луча, ограниченного холстом.
    5. Находим массив объектов, содержащих целочисленные координаты точек на каждой стороне многоугольника.
    6. Перебираем точки на отрезке луче и точки на сторонах многоугольника, сохраняя все совпадающие точки в массив.
    7. Перебираем точки в упомянутом массиве с конца в начало, чтобы убрать соседние точки пересечения отрезка луча и 
    сторон многоугольника, которые находятся на одной и той же Y-координате, но их X-координаты отличаются на 1.
    8. Проверяем не получается ли так, что все точки в упомянутом массиве совпадают с какими-то вершинами 
    многоугольника. Если это так, то возвращаем false, как знак того, что точка находится снаружи многоугольника.
    9. Если количество пересечений отрезка луча и сторон многоугольника нечетное, возвращаем true, как знак того, что 
    ...
    isPointInsidePolygon: function (point, vertices) {
        ...
        /*Проверяем не находится ли точка снаружу "ограничивающей коробки" многоугольника при помощи метода 
        ...
        ограниченный холстом. Поэтому формируем координаты для точки пересечения луча и правой (так как пускаем луч 
        вправо) границы холста, и сохраняем эти данные в переменной "rayIntersectionPoint".*/
        ...
        "findIntPointsOnLineSegment()" и сохраняем их в переменной "rayPoints".*/
        const rayPoints = this.findIntPointsOnLineSegment(point.x, point.y,
            rayCanvasIntersectionPoint.x, rayCanvasIntersectionPoint.y);

        /*Находим массив объектов, содержащих целочисленные координаты точек на каждой стороне многоугольника при помощи 
        метода "findIntPointsOnPolygonEdges()" и сохраняем его в переменную "pointsOnPolygonEdges".*/
        ...
        /*Выводим точки пересечений отрезка луча и сторон многоугольника. Это нужно только для тестирования.*/
        ...
        отрезка луча и сторон многоугольника, которые находятся на одной и той же Y-координате, но их X-координаты 
        отличаются на 1. Эта проверка нужна, чтобы избежать случаев, когда какая-то сторона многоугольника или часть 
        стороны многоугольника совпадает с отрезком луча или частью отрезка луча. Эта проверка игнорирует первую точку в 
        ...
        for (let i = 0; i < intersectionPoints.length; i++) {
            ...
            for (let j = 0; j < vertices.length; j++) {
                /*Если какая-то точка из массива "intersectionPoints" совпадает с какой-то вершиной многоугольника, то
                засчитываем это путем увеличения значения переменной "pointsEqualToPolygonVerticesCount" на 1.*/
        ...
        /*Проверяем не получается ли так, что все точки в массиве "intersectionPoints" совпадают с какими-то вершинами
        многоугольника. Если это так, то возвращаем false, как знак того, что точка находится снаружи многоугольника.*/
    ...
    getPointPositionRelativeToLineSegment: function (
        ...
        находится справа от прямой, содержащей отрезок. В этом случае вовзращаем -1, как знак того, что точка находится 
    ...
    getVectorsRelativeDirection: function (
        ...
        направлены в целом в одну сторону. В этом случае вовзращаем 1, как знак того, что векторы направлены в целом в 
        одну сторону.*/
        ...
        направлены в целом в противоположные стороны. В этом случае вовзращаем 1, как знак того, что векторы направлены 
    ...
    1. Перебираем вершины многоугольников и ищем совпадения их вершин.
